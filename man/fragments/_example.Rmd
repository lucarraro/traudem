...
```{r, fig.alt = "Plot of the catchment contour."}
library(elevatr)
library(sf)
library(terra)
library(traudem)
...
```{r, fig.alt = "Plot of the drainage area, where the number of cells is shown on a log scale with colors ranging from blue to yellow."}
...
The total area of the extracted catchment is `r floor(max(values(ssa),na.rm=T)*cellsize^2/10^6)` km^2.
