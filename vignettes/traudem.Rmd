...
- It provides wrapper commands that allow calling TauDEM methods from R.
...
#> Processes: 1
#> Header read time: 0.007304
#> Data read time: 0.003052
#> Compute time: 0.035886
#> Write time: 0.002312
#> Total time: 0.048554
...
#> PitRemove version 5.3.9
#> Input file /tmp/RtmpdGKRyh/file69f579638367/DEM.tif has projected coordinate system.
#> Nodata value input to create partition from file: nan
#> Nodata value recast to float used in partition raster: nan
#> Processes: 1
#> Header read time: 0.008291
#> Data read time: 0.003158
#> Compute time: 0.024729
#> Write time: 0.001945
#> Total time: 0.038122
#> This run may take on the order of 1 minutes to complete.
#> This estimate is very approximate. 
...
#> a dual quad core Dell Xeon E5405 2.0GHz PC with 16GB RAM.
output
#> [1] "/tmp/RtmpdGKRyh/file69f579638367/DEMfel.tif"
```
...
output
#> [1] "/tmp/RtmpdGKRyh/file69f558d6d3aa/DEMfel.tif"
```
...
output <- taudem_pitremove(file.path(test_dir, "DEM.tif"))
output
#> [1] "/tmp/RtmpdGKRyh/file69f5f1debd0/DEMfel.tif"
```


## Full example: download DEM data and extract a river network

`traudem` can be used in combination with R-package [elevatr](https://cran.r-project.org/web/packages/elevatr/index.html) to extract a river network from elevation data retrieved from the web. 
...
- EPSG code corresponding to the projection used to express coordinate data (see also http://www.epsg-registry.org);
- zoom level at which the DEM data are extracted (see `elevatr` [documentation](https://cran.r-project.org/web/packages/elevatr/vignettes/introduction_to_elevatr.html)).
...
#> Data read time: 0.003557
#> Compute time: 0.025951
#> Write time: 0.001925
#> Total time: 0.039635
#> This run may take on the order of 1 minutes to complete.
#> This estimate is very approximate. 
...
<img src="traudem-unnamed-chunk-8-1.png" alt="Plot of the catchment contour."  />
...
<img src="traudem-unnamed-chunk-9-1.png" alt="Plot of the drainage area, where the number of cells is shown on a log scale with colors ranging from blue to yellow."  />
...
The total area of the extracted catchment is 363 km^2.
